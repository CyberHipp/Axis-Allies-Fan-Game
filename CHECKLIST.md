...
- [ ] Keep asset filenames clean (no spaces/special characters); board image at `assets/board.jpg`.
- [ ] index.html loads hidden board image and canvas with deferred scripts (`data.js`, `game.js`).
- [ ] Acceptance: loads on GitHub Pages and via `python -m http.server`.

## Milestone 1 — Data contract
- [ ] `data.js` defines global `AA` with `rules`, `map` (image, size, territories), `units`, `setup` (turnOrder, ipc, stacks).
- [ ] Acceptance: game boots and draws some stacks on the map.

## Milestone 2 — Unit stats (1942.2 hardcoded)
- [ ] Include full 1942.2 unit table (cost/att/def/move) in `AA.units`.
- [ ] Acceptance: UI "Unit Reference" panel prints these exactly.

## Milestone 3 — Start screen options
- [ ] Start overlay requires Victory Mode selection (VC_STANDARD default, VC_TOTAL, CAPITALS).
- [ ] Map data flags victory cities and capitals; evaluate victory at end of USA turn.
...
- [ ] Implement phases: Purchase → Combat Move → Conduct Combat → Noncombat Move → Mobilize → Collect Income → Next Power.
- [ ] Track `state = { round, currentPower, phase, options, pendingBattles, pendingAmphib }` with `nextPhase()/nextPower()` and validation scaffolding.
- [ ] Acceptance: "Next Phase" cycles correctly and UI shows current power + phase.

## Milestone 5 — Rendering + territory interaction
- [ ] Draw board with pan/zoom, territory polygons, unit counters; hit testing for selection.
- [ ] Acceptance: clicking reliably selects intended territory.

## Milestone 6 — Movement legality
- [ ] Move one unit at a time respecting movement points; Combat vs Noncombat legality; air movement distances.
- [ ] Acceptance: infantry 1, tank 2, ships 2, fighters 4, bombers 6.

## Milestone 7 — Combat core
- [ ] Dice combat for land/sea with logs, casualty choice, retreat handling.
...
- [ ] Export/import JSON for full game state.
